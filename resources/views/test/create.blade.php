...
@section('content')
<br>
<div class="alert mycolor1" role="alert">Test 추가</div>

<form name="form1" method="post" action="{{ route('test.store') }}">
@csrf
...
<table class="table table-sm table-bordered mymargin5">
    ...
    <tr>
        ...
        <td align="left">
            <input type="text" name="coname" value="{{ old('coname') }}" class="form-control form-control-sm" required>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td align="left">
            <input type="date" name="startday" value="{{ old('startday') }}" class="form-control form-control-sm">
        </td>
    </tr>
    <tr>
        ...
        <td align="left">
            <select name="cokind" class="form-select form-select-sm">
                @foreach($a_cokind as $k=>$v)
                    <option value="{{ $k }}" @selected(old('cokind',0)==$k)>{{ $v }}</option>
                @endforeach
            </select>
        </td>
    </tr>
