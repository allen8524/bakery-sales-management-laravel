...
$(function () {
  ...
  $("#ajax_save").on("click", function () {
    ...
    if (!name) { alert("구분명을 입력하세요."); return; }
    ...
    if (mode === "edit") {
      $.ajax({
        ...
        success: function () {
          $("#rowno" + id + " .ajax_edit").text(name).attr("data-name", name);
        }
      });
    } else {
      $.ajax({
        ...
        success: function (data) {
          ...
          $("#table_list").append(
            "<tr id='rowno" + newId + "'>" +
            "  <td>" + newId + "</td>" +
            "  <td><a href='#ajaxModal' data-bs-toggle='modal' data-bs-target='#ajaxModal' class='ajax_edit' data-id='" + newId + "' data-name='" + name + "'>" + name + "</a></td>" +
            "  <td><a href='#' rowno='" + newId + "' class='ajax_del btn btn-sm mycolor1'>삭제</a></td>" +
            "</tr>"
          );
        }
      });
  ...
  $("#table_list").on("click", ".ajax_del", function () {
    ...
    const id = $(this).attr("rowno");
    $.ajax({
      headers: { "X-CSRF-TOKEN": $("meta[name='_token']").attr("content") },
      url: "ajax/" + id,
      type: "POST",
      data: { _method: "DELETE" },
      success: function () { $("#rowno" + id).remove(); }
    });
  });
});
</script>
	...
	<div class="row">
		...
		<div class="col-9" align="right">
			<a href="#ajaxModal" class="ajax_add btn btn-sm mycolor1" data-bs-toggle="modal" data-bs-target="#ajaxModal">추가</a>
		</div>
	</div>
	</form>
	...
	<table class="table table-sm table-bordered table-hover mymargin5" id="table_list">
		<thead>
			<tr>
				...
				<td class="mycolor2" width="80%">구분명</td>
				<td class="mycolor2" width="10%">삭제</td>
			</tr>
		</thead>
			@foreach ($list as $row)
			<tr id="rowno{{$row->id}}">
				<td>{{ $row->id }}</td>
				<td><a href="#ajaxModal" data-bs-toggle="modal" data-bs-target="#ajaxModal" class="ajax_edit" data-id="{{ $row->id }}" data-name="{{ $row->name }}">{{ $row->name }}</a></td>
				<td><a href="#" rowno="{{ $row->id }}" class="ajax_del btn btn-sm mycolor1">삭제</a></td>
			</tr>
		@endforeach
	</table>
